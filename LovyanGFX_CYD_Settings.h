...
public:
  ...
  LGFX(void)
  {
    ...
    // Configure touchscreen control - comment out if not needed
    {
      ...
      cfg.bus_shared = false; // If you are using a bus that is common with the screen, set it to true
      cfg.offset_rotation = 1; // Adjust if display and touch direction do not match. Set a value between 0 and 7.

      cfg.spi_host = -1; // The CYD uses software SPI for the touchscreen

      // Touchscreen GPIO pins on the CYD
      cfg.pin_sclk = 25;
      cfg.pin_mosi = 32;
      cfg.pin_miso = 39;
      cfg.pin_cs   = 33;
